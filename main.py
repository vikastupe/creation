...
class Handler(ControlBrowsers):
    def __init__(self, proxy_ip, proxy_port, proxy_user=None, proxy_password=None, number_provider='5sim'):
        if number_provider not in ('5sim', 'sms-activation-service', 'smspva'):
            raise Exception('logic not implemented for provider: ' + str(number_provider))
        ...
        print(f'number provider: {number_provider}')
        if number_provider == '5sim':
            self.number_provider = Number5Sim()
            ...
            self.number_provider = SmsActivationService()
            if float(self.number_provider.get_balance()) < self.number_provider.min_bal_required:
                raise Exception(f'balance is too low please top up your number provider: {number_provider}, current balance: {self.number_provider.get_balance()}')
    ...
    def run(self):
        try:
            self.goto('https://mail.yahoo.com')
            self.click(search_for='//a[contains(text(), "Sign in")]', element_name='sign in button', critical=False)
            self.click(search_for='//a[@id="createacc"]', element_name='create account button')
            self.wait(2)
            ...
            while cnt:
                ...
                self.wait(2)
                self.click(search_for='//button[@id="reg-sms-button"]', element_name='send sms button', timeout=10)
                self.wait(5)
            ...
            while cnt:
                ...
                if sms_status['status'] == 'success':
                    if sms_status['code']:
                        ...
                        self.click(search_for='//button[@id="verify-code-button"]', element_name='verify otp button')
...
if __name__ == "__main__":
    # obj = Handler('na.qnhpzhea.lunaproxy.net', 12233, proxy_user='user-lu5436108-region-PL', proxy_password='wEZ5xgFmGQ9kNfHa', number_provider='sms-activation-service')
    obj = Handler('1.1.1.1', 12233, number_provider='sms-activation-service')
    obj.run()


# na.qnhpzhea.lunaproxy.net:12233
# USER=user-lu5436108-region- PASSWORD=wEZ5xgFmGQ9kNfHa
